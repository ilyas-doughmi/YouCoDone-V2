<section>
    ...
    <form method="post" action="{{ route('profile.update') }}" class="mt-6 space-y-6">
        ...
        <div>
            ...
            <input id="name" name="name" type="text" class="mt-1 block w-full px-4 py-3 rounded-xl border-gray-200 focus:border-brand-500 focus:ring-brand-500 shadow-sm" value="{{ old('name', $user->name) }}" required autofocus autocomplete="name" />
            @error('name')
        ...
        <div>
            ...
            <input id="email" name="email" type="email" class="mt-1 block w-full px-4 py-3 rounded-xl border-gray-200 focus:border-brand-500 focus:ring-brand-500 shadow-sm" value="{{ old('email', $user->email) }}" required autocomplete="username" />
            @error('email')
                <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
            @enderror
